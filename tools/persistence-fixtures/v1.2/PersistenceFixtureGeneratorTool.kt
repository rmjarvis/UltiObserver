...
private fun writeCompletedArchive(dir: File) {
    val store = freshStore(dir)
    val base = nonDefaultSetup()
    val richSetup = base.copy(
        division = GameDivision.MIXED,
        rules = base.rules
            .withAirQualityGuidelines(true)
            .withHeatLevel(HeatLevel.LEVEL_2),
    )
    check(richSetup.rules.useAirQualityGuidelines)
    check(richSetup.rules.heatLevel == HeatLevel.LEVEL_2)
    check(richSetup.rules.waterBreakMinutes == 4)
    check(richSetup.rules.timeBetweenPointsSeconds == 120)
    check(richSetup.rules.softCapMinutes == 70)
    check(richSetup.rules.hardCapMinutes == 90)
    val richGame = activeGameWithEvents(richSetup, RuleGuidanceMode.FULL)
...
private fun writeCompleteCurrentGame(dir: File) {
    ...
    val guidanceMode = RuleGuidanceMode.NONE
    ...
    check(settings.ruleGuidanceMode == RuleGuidanceMode.NONE)
...
private fun activeGameWithEvents(
    setup: GameState,
